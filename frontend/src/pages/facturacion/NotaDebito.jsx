...
export default function NotaDebito() {
  ...
  const submit = async (e) => {
    ...
    try {
      await api.post('/facturacion/facturas/', {
        tipo: 'ND', tercero: terceroId, fecha,
        observaciones: `Nota Débito — Ref: ${facturas.find(f => String(f.id) === String(facturaRef))?.numero || ''}\n${observaciones}`,
        detalles: lines,
      })
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
      <h1 className="text-2xl font-bold text-slate-800">Nueva Nota Débito</h1>
      ...
      <form onSubmit={submit} className="space-y-4">
        <div className="card space-y-3">
          ...
          <div>
            <label className="label">Observaciones / Motivo *</label>
            <textarea className="input" rows={2} value={observaciones} onChange={e => setObservaciones(e.target.value)} placeholder="Motivo de la nota débito…" required />
          </div>
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="text-right">
            <p className="text-sm text-gray-500">Total ND: <strong>{fmt(totalLineas)}</strong></p>
          </div>
        </div>

        <div className="flex justify-end gap-2">
          <button type="button" onClick={() => navigate('/facturacion')} className="btn-secondary">Cancelar</button>
          <button type="submit" className="btn-primary" disabled={saving || !facturaRef}>{saving ? 'Guardando…' : 'Crear Nota Débito'}</button>
        </div>
      </form>
    </div>
  )
